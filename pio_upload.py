...
dev_ip = None
def get_url(dev_id):
    topic_ip = f'/mqtt/dev/ip/{dev_id}'
    topic_update = f'/mqtt/dev/update/{dev_id}'
    ...
    def on_msg(client, userdata, msg):
        ...
        if (msg.topic == topic_ip):
            ...
            client.publish(topic_update, 'go', qos=1, retain=False)
            time.sleep(2)
            client.disconnect()

    def on_connect(client, userdata, flags, reason_code, properties):
        print('MQTT server connected with result code: ' + str(reason_code))
        if reason_code == 0:
...
def on_upload(source, target, env):
    ...
    upload_dev = env.GetProjectOption('custom_upload_dev', None)
    ...
    auth = None
    upload_url = upload_url.replace("/update", "")
    ...
    with open(firmware_path, 'rb') as firmware:
        ...
        if checkAuthResponse.status_code == 401:
            ...
            print("Server configuration: authentication needed.")
            ...
            auth = None
            print("Server configuration: authentication not needed.")
            try:
